...
export const JusticeUnitPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchContent = async () => {
      try {
        const data = await landingPageService.getPageContent('justice');
        if (data) setContent(data);
      } catch (err) {
        console.error('Failed to load justice page content:', err);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <div className="h-screen w-full flex items-center justify-center bg-[#0a1a0a]">
  ...
  return (
    <div className="min-h-screen bg-[#0a1a0a] text-brand-text font-sans overflow-x-hidden selection:bg-green-500 selection:text-white">
      {/* Animated Background */}
      ...
      <header className="relative h-[80vh] flex items-center justify-center overflow-hidden">
        {content?.heroImages?.length ? (
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <img src={content.heroImages[currentSlide]} className="w-full h-full object-cover" alt="Justice" />
            </motion.div>
          </AnimatePresence>
        ) : (
           <div className="absolute inset-0 bg-gradient-to-br from-[#0a1a0a] via-[#0f2a0f] to-[#0a1a0a]"></div>
        )}
        ...
        <div className="relative z-20 max-w-5xl mx-auto text-center px-8">
          <motion.div 
            ...
            className="inline-flex items-center gap-2 px-3 py-1 bg-green-500/10 border border-green-500/20 rounded-full mb-8 text-green-400"
          >
            <Shield className="w-3 h-3" />
            <span className="text-[9px] font-black uppercase tracking-widest">Public Safety & Order</span>
          </motion.div>
          ...
          >
            {content?.title || <>Fairness <br /> Through <span className="text-green-500">Law</span></>}
          </motion.h1>
          ...
          >
             {content?.body?.substring(0, 200) || "The Baluarte Justice Unit manages peace and order through standardized blotter records, fair mediation processes, and historical case tracking."}
          </motion.p>
        </div>
      </header>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
              {content.quickFacts.map(fact => (
                <div key={fact.id} className="p-6 bg-black/40 border border-white/10 rounded-2xl text-center">
                  ...
                  <p className="text-sm font-black text-green-500 truncate">{fact.value}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <section className="relative z-10 py-24 px-8 border-b border-white/5">
        <div className="max-w-4xl mx-auto text-center">
          <h2 className="text-[10px] font-black uppercase tracking-[0.4em] text-green-500 mb-6">Our Mandate</h2>
          <div className="text-xl md:text-3xl font-black uppercase tracking-tight text-brand-text leading-tight whitespace-pre-wrap">
            {content?.body || "Enforcing justice and maintaining communal harmony through transparent mediation and rigorous documentation. We serve as the primary legal and safety hub for Barangay Baluarte."}
          </div>
        </div>
      </section>
      ...
      {content?.announcements && content.announcements.length > 0 && (
        <section className="relative z-10 py-24 px-8 bg-black/20">
          <div className="max-w-7xl mx-auto">
            <h3 className="text-3xl font-black uppercase tracking-tighter mb-12 text-center text-green-500">Public Bulletins</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {content.announcements.map((ann) => (
                <div key={ann.id} className="p-8 bg-white/5 border border-white/10 rounded-2xl">
                  <Megaphone className="w-6 h-6 text-green-500 mb-6" />
      ...
      {content?.communityHappenings && content.communityHappenings.length > 0 && (
        <section className="relative z-10 py-24 px-8">
          <div className="max-w-7xl mx-auto">
            <h3 className="text-3xl font-black uppercase tracking-tighter mb-12 text-center text-green-500">Safety Initiatives</h3>
              ...
              {content.communityHappenings.map((event) => (
                <div key={event.id} className="group relative aspect-video rounded-3xl overflow-hidden border border-white/10 bg-white/5">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-transparent to-transparent flex flex-col justify-end p-8">
                    <span className="text-[9px] font-black text-green-500 uppercase tracking-widest mb-1">{new Date(event.date).toLocaleDateString()}</span>
